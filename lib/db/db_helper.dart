...
//Multi Screen query Start now:
  ...
  static Future<List<ExpenseModel>>getMobileBillCostExpence(String catagory)async{
    final db=await open();
    final maplist=await db.query(tableExp,where: '$tblExpColCatagory = ? ',whereArgs: [catagory]);
    return List.generate(maplist.length, (index) =>
        ExpenseModel.fromMap(maplist[index]),
  ...
  static Future<void> deleteExpence(ExpenseModel expence) async {
    final db=await open();
    await db.delete(tableExp,where: '$tblExpColId =?',whereArgs: [expence.id]);
    print('done');
